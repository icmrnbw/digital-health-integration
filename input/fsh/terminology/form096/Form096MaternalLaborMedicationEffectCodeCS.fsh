CodeSystem: Form096MaternalLaborMedicationEffectCodeCS
Id: maternal-labor-medication-effect-code-cs
Title: "Maternal Labor Medication Effect CodeSystem"
Description: "Governed local concepts for Form 096, generated from the approved Google Sheet terminology table."
* ^url = "https://terminology.dhp.uz/fhir/chr/CodeSystem/maternal-labor-medication-effect-code-cs"
* ^status = #draft
* ^experimental = true
* ^caseSensitive = true
* ^content = #complete
* ^hierarchyMeaning = #is-a
* ^language = #uz

* #chr-0127-0001 "Yuborilgan dori vositalarining samarasi"
  * ^designation[0].language = #en
  * ^designation[=].value = "Effect of administered medication"
  * ^designation[+].language = #ru
  * ^designation[=].value = "Эффект введённых лекарственных препаратов"
